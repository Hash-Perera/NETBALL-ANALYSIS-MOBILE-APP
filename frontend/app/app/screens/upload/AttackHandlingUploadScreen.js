...
export default function AttackHandlingUploadScreen({ navigation }) {
    ...
    const uploadVideos = async () => {
        ...
        try {
            ...
            formData.append('attack_analysis_correct', {
                uri: correctVideo,
                type: 'video/mp4',
                name: 'attack_analysis_correct.mp4',
            });

            formData.append('attack_analysis_wrong', {
                uri: wrongVideo,
                type: 'video/mp4',
                name: 'attack_analysis_wrong.mp4',
            });
            ...
            const response = await fetch(`${BASEURL}attackanalysis/upload`, {
                method: 'POST',
                body: formData,
                headers: {
            ...
            if (response.ok) {
                await AsyncStorage.setItem('AttackAnalysisId', data.data._id);
                Alert.alert('Success', 'Video uploaded successfully!', [
                    {
                        text: 'OK',
                        onPress: () =>
                            navigation.navigate('AttackAnalyzeScreen', {
                                AttackAnalysisId: data.data._id,
                            }),
                    },
                ]);
            } else {
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.headerText}>Attack Handling Upload</Text>
            </View>
